...
@Component({
  selector: 'app-pr-definitiva-favorable',
  standalone: true,
...
export class PrDefinitivaFavorableComponent {
  ...
  getActoAdminDetail() {
    this.documentosGeneradosService.getDocumentosGenerados(this.actualID, this.actualNif, this.actualConvocatoria, 'doc_prop_res_def_favorable_sin_req')
      .subscribe({
  ...
  generarPDF(jsonObject: any, docFieldToUpdate: string, hayMejoras: number): void {
    ...
    doc.setFontSize(10);
    doc.text(doc.splitTextToSize(jsonObject.antecedentes_5_11, maxTextWidth), marginLeft + 5, 60);
    ...
    this.actoAdminService.sendPDFToBackEnd(formData).subscribe({
      next: (response) => {
        ...
        this.documentosGeneradosService.deleteByIdSolNifConvoTipoDoc( this.actualID, this.actualNif, this.actualConvocatoria, 'doc_prop_res_def_favorable_sin_req')
          .subscribe({
            next: () => {
