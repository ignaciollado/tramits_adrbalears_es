...
export class InformeFavorableComponent {
  ...
  actoAdminName = "informe_favorable_sin_requerimiento"
  ...
  generarPDF(jsonObject: any, docFieldToUpdate: string, hayMejoras: number): void {
    ...
    doc.setFont('helvetica', 'normal');
    doc.text(doc.splitTextToSize(jsonObject.hechos_1_2, maxTextWidth - 5), marginLeft + 5, 125);
    if (hayMejoras > 0) {
      doc.text(doc.splitTextToSize(jsonObject.hechos_3_m, maxTextWidth - 5), marginLeft + 5, 155);
      doc.text(doc.splitTextToSize(jsonObject.hechos_4_5, maxTextWidth - 5), marginLeft + 5, 175);
      doc.setFont('helvetica', 'bold');
      ...
      doc.text(doc.splitTextToSize(jsonObject.hechos_4_5, maxTextWidth - 5), marginLeft + 5, 155);
      doc.setFont('helvetica', 'bold');
