...
export class PrProvisionalDesfavorableComponent {
  ...
  generarPDF(jsonObject: any, docFieldToUpdate: string, hayMejoras: number): void {
    ...
    if (hayMejoras > 0) {
      doc.text(doc.splitTextToSize(jsonObject.antecedentes_4_m, maxTextWidth), marginLeft + 5, 200);
    }
    ...
    doc.setFont('helvetica', 'normal');
    doc.text(doc.splitTextToSize(jsonObject.fundamentos_txt, maxTextWidth), marginLeft + 5, 105);
