...
export class PrProvisionalDesfavorableComponent {
  ...
  actoAdminName: string = "prop_res_prov_desf_sin_req"
  ...
  ngOnInit(): void {
    this.formPRProvDesf = this.fb.group({
  ...
  generateActoAdmin(actoAdministrivoName: string, tipoTramite: string, docFieldToUpdate: string = this.actoAdminName): void {
    ...
    if (this.form.get('fecha_infor_fav_desf')?.value === "0000-00-00 00:00:00" || this.form.get('fecha_infor_fav_desf')?.value === '0000-00-00' || this.form.get('fecha_infor_fav_desf')?.value === null) {
      errorMessage += "- Fecha Firma informe favorable / desfavorable"
      todoOK = false
    }      
    if (!todoOK) {
      alert (errorMessage)
    ...
    this.actoAdminService.getByNameAndTipoTramite(actoAdministrivoName, tipoTramite).subscribe((docDataString: any) => {
      ...
      rawTexto = rawTexto.replace(/%IMPORTETOTALCONVO%/g, this.commonService.formatCurrency(this.importeTotalConvo));
  ...
  generarPDF(jsonObject: any, docFieldToUpdate: string, hayMejoras: number): void {
    ...
    if (hayMejoras > 0) {
      doc.text(doc.splitTextToSize(jsonObject.antecedentes_4_m, maxTextWidth - 5), marginLeft + 5, 200);
    }
    ...
    doc.setFontSize(10);
    doc.text(doc.splitTextToSize(jsonObject.antecedentes_5_7, maxTextWidth - 5), marginLeft + 5, 60);
    doc.setFont('helvetica', 'bold');
    doc.text(doc.splitTextToSize(jsonObject.fundamentos_tit, maxTextWidth), marginLeft, 100);
    doc.setFont('helvetica', 'normal');
    doc.text(doc.splitTextToSize(jsonObject.fundamentos_txt, maxTextWidth - 5), marginLeft + 5, 105);
  ...
  sendActoAdminToSign(nif: string, folder: string, filename: string, extension: string): void {
    ...
    filename = filename.replace(/^doc_/, "")
    filename = `${this.actualIdExp+'_'+this.actualConvocatoria+'_'+filename}`
    this.actoAdminService.getByNameAndTipoTramite('9_propuesta_resolucion_provisional_desfavorable_sin_requerimiento', 'XECS')
