...
@Component({
  selector: 'app-pr-definitiva-desfavorable-con-requerimiento',
  standalone: true,
  ...
  templateUrl: './pr-definitiva-desfavorable-con-requerimiento.component.html',
...
export class PrDefinitivaDesfavorableConRequerimientoComponent {
  ...
  ngOnInit(): void {
    this.actoAdminService.getByNameAndTipoTramite('14_propuesta_resolucion_definitiva_desfavorable_con_requerimiento', 'XECS')
  ...
  generateActoAdmin(actoAdministrivoName: string, lineaAyuda: string, docFieldToUpdate: string = 'doc_prop_res_definitiva_con_req'): void {
    ...
    this.actoAdminService.getByNameAndTipoTramite(actoAdministrivoName, tipoTramite).subscribe((docDataString: any) => {
      ...
      rawTexto = rawTexto.replace(/%FECHA_REC_ENMIENDA%/g, this.commonService.formatDate(this.form.get('fecha_REC_enmienda')?.value));
      ...
      rawTexto = rawTexto.replace(/%FECHANOTIPRPROVISIONAL%/g, this.commonService.formatDate(this.form.get('fecha_not_propuesta_resolucion_prov')?.value));
