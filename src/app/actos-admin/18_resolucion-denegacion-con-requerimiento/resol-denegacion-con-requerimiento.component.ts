...
private expedienteService = inject(ExpedienteService)
  ...
  actoAdminName: string = "res_denegacion_con_req"
  actoAdmin18: boolean = false
  signedBy: string = ""
    ...
    this.documentosGeneradosService.getDocumentosGenerados(this.actualID, this.actualNif, this.actualConvocatoria, "doc_"+this.actoAdminName)
      .subscribe({
        next: (docActoAdmin: DocumentoGeneradoDTO[]) => {
          this.actoAdmin18 = false
          if (docActoAdmin.length === 1) {
            this.actoAdmin18 = true
  ...
  generarPDF(jsonObject: any, docFieldToUpdate: string = this.actoAdminName, hayMejoras: number): void {
    ...
    doc.setFont('helvetica', 'normal');
    doc.text(doc.splitTextToSize(jsonObject.antecedentes_1_8, maxTextWidth - 5), marginLeft + 5, 140);
