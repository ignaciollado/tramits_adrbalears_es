...
export class InformeDesfavorableComponent {
  ...
  actoAdminName: string = "informe_desfavorable_sin_requerimiento"
  ...
  ngOnInit(): void {
    this.actoAdminService.getByNameAndTipoTramite('6_informe_desfavorable_sin_requerimiento', 'XECS')
  ...
  generarPDF(jsonObject: any, docFieldToUpdate: string, hayMejoras: number): void {
    ...
    if (hayMejoras > 0) {
      ...
      doc.text(doc.splitTextToSize(jsonObject.hechos_4, maxTextWidth - 5), marginLeft + 5, 170);
    } else {
      doc.text(doc.splitTextToSize(jsonObject.hechos_4, maxTextWidth - 5), marginLeft + 5, 155);
    }
