...
export class PrDefinitivaFavorableConRequerimientoComponent {
  ...
  actoAdminName:string = "prop_res_def_favorable_con_req"
  ...
  generateActoAdmin(actoAdministrivoName: string, tipoTramite: string, docFieldToUpdate: string = this.actoAdminName): void {
    ...
    this.actoAdminService.getByNameAndTipoTramite(actoAdministrivoName, tipoTramite).subscribe((docDataString: any) => {
      ...
      rawTexto = rawTexto.replace(/%FECHAREQ%/g, this.commonService.formatDate(this.form.get('fecha_requerimiento')?.value));
        ...
        if (nMejoras.total_mejoras > 0) {
          ...
          return this.mejorasSolicitudService.obtenerUltimaMejoraSolicitud(this.actualID).pipe(
            tap((ultimaMejora: MejoraSolicitudDTO) => {
              ...
              rawTexto = rawTexto.replace(/%EEE%/g, String("12. "))
              rawTexto = rawTexto.replace(/%FFF%/g, String("13. "))
            })
          );
        } else {
            ...
            rawTexto = rawTexto.replace(/%EEE%/g, String("11. "))
            rawTexto = rawTexto.replace(/%FFF%/g, String("12. "))            
          return of(null);
        }
      }),
      tap(() => {
  ...
  generarPDF(jsonObject: any, docFieldToUpdate: string, hayMejoras: number): void {
    ...
    if (hayMejoras > 0) {
      doc.text(doc.splitTextToSize(jsonObject.antecedentes_4_m, maxTextWidth - 5), marginLeft + 5, 211);
    }
    ...
    doc.setFontSize(10);
    doc.text(doc.splitTextToSize(jsonObject.antecedentes_5_13, maxTextWidth - 5), marginLeft + 5, 60);
  ...
  sendActoAdminToSign(nif: string, folder: string, filename: string, extension: string): void {
    ...
    filename = filename.replace(/^doc_/, "")
    filename = `${this.actualIdExp+'_'+this.actualConvocatoria+'_'+filename}`
    this.actoAdminService.getByNameAndTipoTramite('12_propuesta_resolucion_definitiva_favorable_con_requerimiento', 'XECS')
