...
export class ResolucionDePagoYJustificacionAdrIsbaComponent {
  ...
  ngOnInit(): void {
    this.actoAdminService.getByNameAndTipoTramite('isba_11_resolucion_pago_y_justificacion', 'ADR-ISBA')
  ...
  generateActoAdmin(actoAdministrativoName: string, tipoTramite: string, docFieldToUpdate: string): void {
    ...
    this.actoAdminService.getByNameAndTipoTramite(actoAdministrativoName, tipoTramite)
      .subscribe((docDataString: ActoAdministrativoDTO) => {
        ...
        doc.setFont('helvetica', 'normal');
        doc.text(doc.splitTextToSize(jsonObject.propuesta_tit, maxTextWidth), marginLeft, 70);
  ...
  private tieneTodosLosCamposRequeridos(): void {
    ...
    if (!fecha_REC_justificacion?.trim() || fecha_REC_justificacion?.trim() === "0000-00-00") {
