...
export class PrDefinitivaFavorableService {
  ...
  init(item: any) {
    this.bindData(item);
    if (!item.doc_prop_res_definitiva_adr_isba || item.doc_prop_res_definitiva_adr_isba === "0") {
      this.configGlobal.getActive().pipe(
        ...
        switchMap(() => this.actoAdminService.getByNameAndTipoTramite(
          'isba_7_propuesta_resolucion_def_favorable',
          'ADR-ISBA'
        ))
      ).subscribe({
        next: (docDataString: ActoAdministrativoDTO) => {
          ...
          this.generateActoAdmin('prop_res_definitiva_adr_isba');
        },
  ...
  // Vinculo las propiedades del componente con los datos del expediente encapsulando la funcionalidad
  bindData(item: any): void {
    this.actualID = item.id;
    this.actualIdExp = item.idExp;
    this.actualEmpresa = item.empresa;
    this.actualConvocatoria = item.convocatoria;
    this.actualNif = item.nif;
    this.actualTipoTramite = item.tipo_tramite;
  ...
  generateActoAdmin(docFieldToUpdate: string): void {
    ...
    this.actoAdminService.sendPDFToBackEnd(formData).subscribe({
      next: (response) => {
        ...
        this.documentosGeneradosService.deleteByIdSolNifConvoTipoDoc(this.actualID, this.actualNif, this.actualConvocatoria, 'doc_prop_res_definitiva_adr_isba')
        .subscribe({
          next: () => {
    ...
    this.documentosGeneradosService.create(this.docGeneradoInsert).subscribe({
      next: (resp: any) => {
        ...
        if (this.lastInsertId) {
          this.expedienteService.updateFieldExpediente(this.actualID, `doc_${docFieldToUpdate}`, String(this.lastInsertId))
            .subscribe({
              next: (response: any) => {
                const mensaje = response?.message || `✅ Prop Definitiva Favorable sin requerimiento generado para ${this.actualEmpresa}`;
              ...
              error: (updateErr) => {
                const updateErrorMsg =
                  ...
                  `⚠️ Ha habido un error intentando generar automáticamente la Prop. Definitiva Favorable sin requerimiento para ${this.actualEmpresa}`;
      ...
      error: (insertErr) => {
        const insertErrorMsg =
          ...
          `❌ Error al guardar la Prop. Definitiva Favorable sin requerimiento para ${this.actualEmpresa}.`;
    ...
    this.viafirmaService.createSignatureRequest(payload)
      ...
      .subscribe({
        next: (res) => {
          ...
          this.commonService.showSnackBar(id ? `Solicitud de firma creada. ID: ${id} y enviada a la dirección: ${payload.adreca_mail} para Prop. Definitiva Favorable sin requerimiento para ${this.actualEmpresa}` : 'Solicitud de firma creada correctamente');
