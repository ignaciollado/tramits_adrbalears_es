...
export class PrRevocacionPorNoJustificarAdrIsbaService {
  ...
  init(expediente: any, form: any) {
    ...
    if (expediente) {
      ...
      this.configGlobal.getActive().pipe(
        ...
        switchMap(() => this.actoAdminService.getByNameAndTipoTramite('isba_16_propuesta_resolucion_revocacion_no_justificar', 'ADR-ISBA'))
      ).subscribe({
        ...
        error: (err) => console.error('Error inicializando Prop. Rev. por no justificar', err)
      })
  ...
  getActoAdminDetail(): void {
    this.documentosGeneradosService.getDocumentosGenerados(this.actualID, this.actualNif, this.actualConvocatoria, 'doc_prop_res_revocacion_por_no_justificar')
      .subscribe({
  ...
  sendActoAdminToSign(): void {
    ...
    const filename = `${this.actualIdExp}_${this.actualConvocatoria}_prop_res_revocacion_por_no_justificar.pdf`;
  ...
  generateActoAdmin(): void {
    ...
    const docFieldToUpdate: string = "prop_res_revocacion_por_no_justificar";
    ...
    this.actoAdminService.sendPDFToBackEnd(formData).subscribe({
      next: (response) => {
        ...
        this.documentosGeneradosService.deleteByIdSolNifConvoTipoDoc(this.actualID, this.actualNif, this.actualConvocatoria, 'doc_prop_res_revocacion_por_no_justificar')
          .subscribe({
            next: () => {
  ...
  private tieneTodosLosCamposRequeridos(): void {
    ...
    if (!fecha_notificacion_resolucion?.trim() || fecha_notificacion_resolucion?.trim() === "0000-00-00 00:00:00") {
