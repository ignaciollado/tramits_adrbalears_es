...
export class ResolucionDePagoYJustificacionAdrIsbaService {
  ...
  init(expediente: any, form: any) {
    ...
    if (expediente) {
      ...
      this.configGlobal.getActive().pipe(
        ...
        switchMap(() => this.actoAdminService.getByNameAndTipoTramite('isba_11_resolucion_pago_y_justificacion', 'ADR-ISBA'))
      ).subscribe({
        ...
        error: (err) => console.error('Error inicializando Res. de pago y justificación', err)
      })
  ...
  getActoAdminDetail(): void {
    this.documentosGeneradosService.getDocumentosGenerados(this.actualID, this.actualNif, this.actualConvocatoria, 'doc_res_pago_y_justificacion_adr_isba')
      .subscribe({
  ...
  sendActoAdminToSign(): void {
    ...
    const filename = `${this.actualIdExp}_${this.actualConvocatoria}_res_pago_y_justificacion_adr_isba.pdf`;
  ...
  generateActoAdmin(): void {
    ...
    const docFieldToUpdate: string = "res_pago_y_justificacion_adr_isba";
    ...
    rawTexto = rawTexto.replace(/%IMPORTE%/g, formattedImporte_prestamo);
    rawTexto = rawTexto.replace(/%FECHA_PROP_RESOL_PROVISIONAL%/g, formattedFecha_firma_propuesta_resolucion_prov);
    rawTexto = rawTexto.replace(/%FECHA_NOTIF_PRPROV%/g, formattedFecha_not_propuesta_resolucion_prov);
    rawTexto = rawTexto.replace(/%FECHA_PR_DEFINITIVA%/g, formattedFecha_firma_propuesta_resolucion_def);
    rawTexto = rawTexto.replace(/%FECHA_NOTIF_PRPDEF%/g, formattedFecha_not_propuesta_resolucion_def);
    rawTexto = rawTexto.replace(/%FECHA_RESOL_CONCE%/g, formattedFecha_firma_resol_concesion);
    ...
    doc.setFontSize(8);
    doc.text(doc.splitTextToSize("Document: resolució de pagament i justificació", maxTextWidth), x, 45);
    ...
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(9);
    doc.text(doc.splitTextToSize(jsonObject.fets_1_2_3_4_5_6, maxTextWidth), marginLeft + 5, 110);
    ...
    doc.setFontSize(9);
    doc.text(doc.splitTextToSize(jsonObject.fets_7_8, maxTextWidth), marginLeft + 5, 60);

    doc.setFontSize(10);
    doc.setFont('helvetica', 'bold');
    doc.text(doc.splitTextToSize(jsonObject.fundamentosDeDerecho_tit, maxTextWidth), marginLeft, 116);

    doc.setFontSize(9)
    doc.setFont('helvetica', 'normal');
    doc.text(doc.splitTextToSize(jsonObject.fundamentosDeDerechoTxt, maxTextWidth), marginLeft + 5, 126);
    ...
    doc.setFontSize(9)
    doc.text(doc.splitTextToSize(jsonObject.propuesta_tit, maxTextWidth), marginLeft, 68);
    doc.setFont('helvetica', 'normal');
    doc.text(doc.splitTextToSize(jsonObject.resolucion, maxTextWidth), marginLeft + 5, 76);
    ...
    this.actoAdminService.sendPDFToBackEnd(formData).subscribe({
      next: (response) => {
        ...
        this.documentosGeneradosService.deleteByIdSolNifConvoTipoDoc(this.actualID, this.actualNif, this.actualConvocatoria, 'doc_res_pago_y_justificacion_adr_isba')
          .subscribe({
            next: () => {
  ...
  private tieneTodosLosCamposRequeridos(): void {
    ...
    const ref_REC_justificacion = this.form.get('ref_REC_justificacion')?.value;
    ...
    if (!fecha_firma_res?.trim() || fecha_firma_res?.trim() === "0000-00-00") {
      this.camposVacios.push('FORM.FECHA_FIRMA_RES')
    }

    if (!fecha_notificacion_resolucion?.trim() || fecha_notificacion_resolucion?.trim() === "0000-00-00") {
    ...
    if (!ref_REC_justificacion?.trim()) {
      this.camposVacios.push('FORM.REF_REC_JUSTIFICACION')
    }
