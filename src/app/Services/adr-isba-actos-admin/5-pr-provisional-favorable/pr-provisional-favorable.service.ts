...
export class PrProvisionalFavorableAdrIsbaService {
  ...
  init(expediente: any, form: any) {
    ...
    if (expediente) {
      ...
      this.configGlobal.getActive().pipe(
        ...
        switchMap(() => this.actoAdminService.getByNameAndTipoTramite('isba_5_propuesta_resolucion_prov_favorable', 'ADR-ISBA'))
      ).subscribe({
        ...
        error: (err) => console.error('Error inicializando Pr. Provisional Favorable sin requerimietno', err)
      })
  ...
  getActoAdminDetail(): void {
    this.documentosGeneradosService.getDocumentosGenerados(this.actualID, this.actualNif, this.actualConvocatoria, 'doc_prop_res_provisional_adr_isba')
      .subscribe({
  ...
  sendActoAdminToSign(): void {
    ...
    const filename = `${this.actualIdExp}_${this.actualConvocatoria}_prop_res_provisional_adr_isba.pdf`;
  ...
  generateActoAdmin(): void {
    ...
    const docFieldToUpdate: string = 'prop_res_provisional_adr_isba';
    ...
    doc.setFontSize(8);
    doc.text("Document: proposta de resolució provisional", x, 45);
    ...
    doc.text(doc.splitTextToSize(jsonObject.fundamentosDeDerechoTxt_2, maxTextWidth), marginLeft + 5, 128);
    ...
    doc.setFont('helvetica', 'normal');
    doc.text(doc.splitTextToSize(jsonObject.propuestaresoluciondef_1_2_3_4_5_6, maxTextWidth), marginLeft + 5, 70);
    ...
    this.actoAdminService.sendPDFToBackEnd(formData).subscribe({
      next: (response) => {
        ...
        this.documentosGeneradosService.deleteByIdSolNifConvoTipoDoc(this.actualID, this.actualNif, this.actualConvocatoria, 'doc_prop_res_provisional_adr_isba')
          .subscribe({
            next: () => {
