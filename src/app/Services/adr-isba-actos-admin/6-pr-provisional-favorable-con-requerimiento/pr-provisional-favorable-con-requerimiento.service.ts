...
export class PrProvisionalFavorableConRequerimientoAdrIsbaService {
  ...
  init(expediente: any, form: any) {
    ...
    if (expediente) {
      ...
      this.configGlobal.getActive().pipe(
        ...
        switchMap(() => this.actoAdminService.getByNameAndTipoTramite('isba_6_propuesta_resolucion_prov_favorable_con_requerimiento', 'ADR-ISBA'))
      ).subscribe({
  ...
  getActoAdminDetail(): void {
    this.documentosGeneradosService.getDocumentosGenerados(this.actualID, this.actualNif, this.actualConvocatoria, 'doc_prop_res_provisional_con_requerimiento_adr_isba')
      .subscribe({
  ...
  sendActoAdminToSign(): void {
    ...
    const filename = `${this.actualIdExp}_${this.actualConvocatoria}_prop_res_provisional_con_requerimiento_adr_isba.pdf`;
  ...
  generateActoAdmin(): void {
    ...
    const docFieldToUpdate: string = "prop_res_provisional_con_requerimiento_adr_isba"
    ...
    this.actoAdminService.sendPDFToBackEnd(formData).subscribe({
      next: (response) => {
        ...
        this.documentosGeneradosService.deleteByIdSolNifConvoTipoDoc(this.actualID, this.actualNif, this.actualConvocatoria, 'doc_prop_res_provisional_con_requerimiento_adr_isba')
          .subscribe({
            next: () => {
