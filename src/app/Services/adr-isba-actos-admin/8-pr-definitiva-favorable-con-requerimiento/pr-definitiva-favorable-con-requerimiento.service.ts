...
export class PrDefinitivaFavorableConRequerimientoAdrIsbaService {
  ...
  init(expediente: any, fromExpedientList: boolean, form?: any) {
    ...
    if (expediente) {
      ...
      this.configGlobal.getActive().pipe(
        ...
        switchMap(() => this.actoAdminService.getByNameAndTipoTramite('isba_8_propuesta_resolucion_def_favorable_con_requerimiento', 'ADR-ISBA'))
      ).subscribe({
        ...
        error: (err) => console.error('Error inicializando Pr. definitiva favorable con requerimiento', err)
      })
  ...
  sendActoAdminToSign(): void {
    ...
    const filename = `${this.actualIdExp}_${this.actualConvocatoria}_prop_res_definitiva_con_requerimiento_adr_isba.pdf`;
  ...
  generateActoAdmin(): void {
    ...
    const docFieldToUpdate = 'prop_res_definitiva_con_requerimiento_adr_isba';
