...
export class ResolucionConcesionAdrIsbaService {
  ...
  // flujoTerminado (Para indicar al detail que se ha acabado el flujo de servicio)
  private flujoTerminadoSubject = new BehaviorSubject<boolean>(false);
  ...
  // Flag para detectar si es automático o no
  automatic!: boolean;
  ...
  init(expediente: any, form: any, automatic: boolean) {
    this.resetSubjects();
    if (form) {
      this.form = form;
    }
    if (expediente) {
      ...
      this.configGlobal.getActive().pipe(
        ...
        switchMap(() => this.actoAdminService.getByNameAndTipoTramite('isba_9_resolucion_concesion', 'ADR-ISBA'))
      ).subscribe({
        next: (docDataString: ActoAdministrativoDTO) => {
          ...
          if (automatic) {
            ...
            this.generateActoAdmin();
          } else {
            this.getActoAdminDetail(); // Carga de expediente previamente generado
          }

        },
        error: (err) => console.error('Error inicializando Resolución Concesión', err)
      })
  ...
  getActoAdminDetail(): void {
    this.documentosGeneradosService.getDocumentosGenerados(this.actualID, this.actualNif, this.actualConvocatoria, 'doc_res_concesion_adr_isba')
      .subscribe({
  ...
  sendActoAdminToSign(): void {
    ...
    const filename = `${this.actualIdExp}_${this.actualConvocatoria}_res_concesion_adr_isba.pdf`;
    ...
    this.viafirmaService.createSignatureRequest(payload)
      ...
      .subscribe({
        next: (res) => {
          ...
          this.commonService.showSnackBar(id ? `Solicitud de firma creada. ID: ${id} y enviada a la dirección: ${payload.adreca_mail} para Resolución de Concesión` : 'Solicitud de firma creada correctamente');
          if (this.automatic) {
  ...
  generateActoAdmin(): void {
    ...
    const docFieldToUpdate: string = "res_concesion_adr_isba";
    ...
    this.actoAdminService.sendPDFToBackEnd(formData).subscribe({
      next: (response) => {
        ...
        this.documentosGeneradosService.deleteByIdSolNifConvoTipoDoc(this.actualID, this.actualNif, this.actualConvocatoria, 'doc_res_concesion_adr_isba')
          .subscribe({
            next: () => {
    ...
    this.documentosGeneradosService.create(this.docGeneradoInsert).subscribe({
        ...
        if (this.lastInsertId) {
          this.expedienteService
            ...
            .subscribe({
              next: (response: any) => {
                const mensaje =
                  response?.message || `✅ Resolución de Concesión generado automáticamente`;
              ...
              error: (updateErr) => {
                const updateErrorMsg =
                  ...
                  '⚠️ Ha habido un error intentando generar automáticamente la Resolución de Concesión.';
      ...
      error: (insertErr) => {
        const insertErrorMsg =
          ...
          '❌ Error al guardar la Resolución de Concesión';
