...
export class InformeFavorableAdrIsbaService {
  ...
  init(expediente: any, form: any) {
    ...
    if (expediente) {
      ...
      this.configGlobal.getActive().pipe(
        ...
        switchMap(() => this.actoAdminService.getByNameAndTipoTramite('isba_3_informe_favorable_sin_requerimiento', 'ADR-ISBA'))
      ).subscribe({
        ...
        error: (err) => console.error('Error inicializando Informe Favorable sin requerimiento', err)
      })
  ...
  getActoAdminDetail(): void {
    this.documentosGeneradosService.getDocumentosGenerados(this.actualID, this.actualNif, this.actualConvocatoria, 'doc_informe_favorable_sin_requerimiento_adr_isba')
      .subscribe({
  ...
  sendActoAdminToSign(): void {
    ...
    const filename = `${this.actualIdExp}_${this.actualConvocatoria}_informe_favorable_sin_requerimiento_adr_isba.pdf`;
  ...
  // Generación acto administrativo
  generateActoAdmin(): void {
    this.tieneTodosLosCamposRequeridos();
    if (this.faltanCampos) { return; }

    const docFieldToUpdate: string = 'informe_favorable_sin_requerimiento_adr_isba';
    ...
    this.actoAdminService.sendPDFToBackEnd(formData).subscribe({
      next: (response) => {
        ...
        this.documentosGeneradosService.deleteByIdSolNifConvoTipoDoc(this.actualID, this.actualNif, this.actualConvocatoria, 'doc_informe_favorable_sin_requerimiento_adr_isba')
          .subscribe({
            next: () => {
