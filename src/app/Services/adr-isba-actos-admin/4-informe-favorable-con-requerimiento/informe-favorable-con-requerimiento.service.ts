...
export class InformeFavorableConRequerimientoAdrIsbaService {
  ...
  init(expediente: any, form: any) {
    ...
    if (expediente) {
      ...
      this.configGlobal.getActive().pipe(
        ...
        switchMap(() => this.actoAdminService.getByNameAndTipoTramite('isba_4_informe_favorable_con_requerimiento', 'ADR-ISBA'))
      ).subscribe({
  ...
  sendActoAdminToSign(): void {
    ...
    const filename = `${this.actualIdExp}_${this.actualConvocatoria}_informe_favorable_con_requerimiento_adr_isba.pdf`;
  ...
  generateActoAdmin(): void {
    ...
    const docFieldToUpdate: string = "informe_favorable_con_requerimiento_adr_isba";
