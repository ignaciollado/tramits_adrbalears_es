...
export class ResolucionConcesionConRequerimientoAdrIsbaService {
  ...
  init(expediente: any, form: any, automatic: boolean) {
    ...
    if (expediente) {
      ...
      this.configGlobal.getActive().pipe(
        ...
        switchMap(() => this.actoAdminService.getByNameAndTipoTramite('isba_10_resolucion_concesion_con_requerimiento', 'ADR-ISBA'))
      ).subscribe({
        ...
        error: (err) => console.error('Error inicializando res. concesion con requerimiento', err)
      })
  ...
  getActoAdminDetail(): void {
    this.documentosGeneradosService.getDocumentosGenerados(this.actualID, this.actualNif, this.actualConvocatoria, 'doc_res_concesion_con_requerimiento_adr_isba')
      .subscribe({
  ...
  sendActoAdminToSign(): void {
    ...
    const filename = `${this.actualIdExp}_${this.actualConvocatoria}_res_concesion_con_requerimiento_adr_isba.pdf`;
  ...
  generateActoAdmin(): void {
    ...
    const docFieldToUpdate: string = "res_concesion_con_requerimiento_adr_isba"
