...
export class PrDefinitivaFavorableAdrIsbaService {
  ...
  // Flag para identificar si es desde listado o detalle
  fromExpedientList: boolean = false;
  ...
  init(expediente: any, fromExpedientList: boolean, form?: any) {
    this.resetSubjects();
    if (form) {
      this.form = form;
    }
    if (expediente) {
      ...
      this.configGlobal.getActive().pipe(
        ...
        switchMap(() => this.actoAdminService.getByNameAndTipoTramite('isba_7_propuesta_resolucion_def_favorable', 'ADR-ISBA'))
      ).subscribe({
        next: (docDataString: ActoAdministrativoDTO) => {
          this.docDataString = docDataString;
          this.signedBy = this.docDataString.signedBy;

          if (fromExpedientList) {
            this.fromExpedientList = fromExpedientList;
            this.generateActoAdmin(); // Genero el expediente automáticamente. Automatismo 1
          } else {
            this.getActoAdminDetail(); // Carga de expediente previamente generado
          }
        },
        error: (err) => console.error('Error inicializando Pr. definitiva favorable sin requerimiento', err)
      })
  ...
  getActoAdminDetail(): void {
    this.documentoGeneradosService.getDocumentosGenerados(this.actualID, this.actualNif, this.actualConvocatoria, 'doc_prop_res_definitiva_adr_isba')
      .subscribe({
  ...
  sendActoAdminToSign(): void {
    ...
    const filename = `${this.actualIdExp}_${this.actualConvocatoria}_prop_res_definitiva_adr_isba.pdf`;
    ...
    this.viafirmaService.createSignatureRequest(payload)
      ...
      .subscribe({
        next: (res) => {
          ...
          this.commonService.showSnackBar(id ? `Solicitud de firma creada. ID: ${id} y enviada a la dirección: ${payload.adreca_mail}. Prop. Definitiva Favorable con requerimiento para ${this.actualEmpresa}` : 'Solicitud de firma creada correctamente');
          if (typeof sendDate === "number") {
            this.expedienteService.updateFieldExpediente(this.actualID, 'fecha_not_propuesta_resolucion_def_sended', this.commonService.convertUnixToHumanDate(sendDate, true)).subscribe();
          }

          if (!this.fromExpedientList) {
            this.viewSignState(this.publicAccessId);
          }
  ...
  generateActoAdmin(): void {
    ...
    const docFieldToUpdate = 'prop_res_definitiva_adr_isba';
    ...
    const formattedFecha_infor = formatDate(this.expediente.fecha_infor_fav_desf, 'dd/MM/yyyy', 'es-ES');
    const formattedFecha_aval_idi_isba = formatDate(this.expediente.fecha_aval_idi_isba, 'dd/MM/yyyy', 'es-ES');
    const formattedFecha_firma_propuesta_resolucion_prov = formatDate(this.expediente.fecha_firma_propuesta_resolucion_prov, 'dd/MM/yyyy', 'es-ES');
    const formattedFecha_not_propuesta_resolucion_prov = formatDate(this.expediente.fecha_not_propuesta_resolucion_prov, 'dd/MM/yyyy', 'es-ES');
    ...
    rawTexto = rawTexto.replace(/%AÑOS_DURACION_AVAL%/g, this.expediente.plazo_aval_idi_isba);
    ...
    this.actoAdminService.sendPDFToBackEnd(formData).subscribe({
      next: (response) => {
        ...
        this.documentoGeneradosService.deleteByIdSolNifConvoTipoDoc(this.actualID, this.actualNif, this.actualConvocatoria, 'doc_prop_res_definitiva_adr_isba')
          .subscribe({
            next: () => {
