...
export class ResRevocacionPorNoJustificarArdIsbaService {
  ...
  // noRevocationReasonText
  private noRevocationReasonTextSubject = new BehaviorSubject<boolean>(true);
  noRevocationReasonText$ = this.noRevocationReasonTextSubject.asObservable();
  ...
  // Si se introduce por primera vez
  ...
  init(expediente: any, form: any) {
    ...
    if (expediente) {
      ...
      this.configGlobal.getActive().pipe(
        ...
        switchMap(() => this.actoAdminService.getByNameAndTipoTramite('isba_17_resolucion_revocacion_no_justificar', 'ADR-ISBA'))
      ).subscribe({
        ...
        error: (err) => console.error('Error inicializando Res. Revocacion por no justificar', err)
      })
    }
  }
  ...
  sendActoAdminToSign(): void {
    ...
    const filename = `${this.actualIdExp}_${this.actualConvocatoria}_resolucion_revocacion_no_justificar.pdf`;
  ...
  // Guardado de motivo
  saveReasonRevocation(formRevocacion: any): void {
    const motivo = formRevocacion.get('motivoResolucionRevocacionPorNoJustificar')?.value;
    console.log(motivo)
    if (formRevocacion.valid) {
      this.motivoResolucionRevocacionPorNoJustificar = motivo;
      this.expedienteService.updateFieldExpediente(this.actualID, 'motivoResolucionRevocacionPorNoJustificar', motivo).subscribe();
      this.noRevocationReasonTextSubject.next(false);
      this.actoAdminSubject.next(false);

      // Añado borrado automático de documento si cambia el motivo
      this.documentoGeneradosService.deleteByIdSolNifConvoTipoDoc(this.actualID, this.actualNif, this.actualConvocatoria, 'doc_resolucion_revocacion_no_justificar').subscribe({
  ...
  generateActoAdmin(): void {
    ...
    const docFieldToUpdate: string = 'resolucion_revocacion_no_justificar';
    ...
    rawTexto = rawTexto.replace(/%TEXTO_LIBRE%/g, this.motivoResolucionRevocacionPorNoJustificar?.toLowerCase() || this.form.get('motivoResolucionRevocacionPorNoJustificar')?.value.toLowerCase());
    ...
    doc.text(doc.splitTextToSize(jsonObject.antecedentes, maxTextWidth), marginLeft, 110);
    doc.setFont('helvetica', 'normal');
    doc.text(doc.splitTextToSize(jsonObject.antecedentes_1_5, maxTextWidth), marginLeft + 5, 120);
  ...
  private resetSubjects(): void {
    this.actoAdminSubject.next(false);
    this.noRevocationReasonTextSubject.next(true);
