...
export class ResolDenegacionConRequerimientoIlsService {
  ...
  init(expediente: any, form: any) {
    ...
    if (expediente) {
      ...
      this.configGlobal.getActive().pipe(
        ...
        switchMap(() => this.actoAdminService.getByNameAndTipoTramite('ILS_6_resolucion_denegacion_con_requerimiento', 'ILS'))
      ).subscribe({
        ...
        error: (err) => console.error('Error inicializando Res. Denegación con requerimiento', err)
      })
  ...
  sendActoAdminToSign(): void {
    ...
    const filename = `${this.actualIdExp}_${this.actualConvocatoria}_res_denegacion_con_req_ils.pdf`;
  ...
  generateActoAdmin(): void {
    ...
    const docFieldToUpdate: string = "res_denegacion_con_req_ils";
