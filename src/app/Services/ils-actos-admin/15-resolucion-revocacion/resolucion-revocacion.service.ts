...
export class ResolucionRevocacionIlsService {
  ...
  init(expediente: any, form: any) {
    ...
    if (expediente) {
      ...
      this.configGlobal.getActive().pipe(
        ...
        switchMap(() => this.actoAdminService.getByNameAndTipoTramite('ILS_15_renovacion_resolucion_revocacion', 'ILS'))
      ).subscribe({
        ...
        error: (err) => console.error('Error inicializando Resolución Revocación', err)
      })
  ...
  getActoAdminDetail(): void {
    this.documentosGeneradosService.getDocumentosGenerados(this.actualID, this.actualNif, this.actualConvocatoria, 'doc_renovacion_resolucion_revocacion_ils')
      .subscribe({
  ...
  sendActoAdminToSign(): void {
    ...
    const filename = `${this.actualIdExp}_${this.actualConvocatoria}_renovacion_resolucion_revocacion_ils.pdf`;
  ...
  generateActoAdmin(): void {
    ...
    const docFieldToUpdate: string = "renovacion_resolucion_revocacion_ils";
    ...
    const formattedFecha_infor_desf_renov = formatDate(this.form.get('fecha_infor_desf_renov')?.value, 'dd/MM/yyyy', 'es-ES');
    ...
    rawTexto = rawTexto.replace(/%FECHAINFDESRENOVACION%/g, formattedFecha_infor_desf_renov);
    ...
    doc.setFontSize(8);
    doc.text("Document: resolució renovació revocació", marginLeft + 110, 45);
    ...
    this.actoAdminService.sendPDFToBackEnd(formData).subscribe({
      next: (response) => {
        ...
        this.documentosGeneradosService.deleteByIdSolNifConvoTipoDoc(this.actualID, this.actualNif, String(this.actualConvocatoria), 'doc_renovacion_resolucion_revocacion_ils')
          .subscribe({
            next: () => {
  ...
  private tieneTodosLosCamposRequeridos(): void {
    ...
    const fecha_infor_desf_renov = this.form.get('fecha_infor_desf_renov')?.value;
    ...
    if (!fecha_infor_desf_renov?.trim() || fecha_infor_desf_renov?.trim() === "0000-00-00") {
      this.camposVacios.push('FORM.RENOVATION-UNFAVORABLE-DATE')
    }
