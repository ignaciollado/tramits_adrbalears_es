...
export class RenovInformeDesfavorableConRequerimientoIlsService {
  ...
  init(expediente: any, form: any) {
    ...
    if (expediente) {
      ...
      this.configGlobal.getActive().pipe(
        ...
        switchMap(() => this.actoAdminService.getByNameAndTipoTramite('ILS_12_renovacion_informe_desfavorable_con_requerimiento', 'ILS'))
      ).subscribe({
        ...
        error: (err) => console.error('Error inicializando Renov. Informe Desfavorable con requerimiento', err)
      })
  ...
  getActoAdminDetail(): void {
    this.documentosGeneradosService.getDocumentosGenerados(this.actualID, this.actualNif, this.actualConvocatoria, 'doc_renovacion_informe_desfavorable_ils')
      .subscribe({
  ...
  sendActoAdminToSign(): void {
    ...
    const filename = `${this.actualIdExp}_${this.actualConvocatoria}_renovacion_informe_desfavorable_ils.pdf`;
  ...
  generateActoAdmin(): void {
    ...
    const docFieldToUpdate: string = "renovacion_informe_desfavorable_ils";
    ...
    doc.setFontSize(8);
    doc.text("Document: renovació informe desfavorable", marginLeft + 110, 45);
    ...
    this.actoAdminService.sendPDFToBackEnd(formData).subscribe({
      next: (response) => {
        ...
        this.documentosGeneradosService.deleteByIdSolNifConvoTipoDoc(this.actualID, this.actualNif, String(this.actualConvocatoria), 'doc_renovacion_informe_desfavorable_ils')
          .subscribe({
            next: () => {
