...
export class ResolConcesionAdhesionIlsService {
  ...
  init(expediente: any, form: any) {
    ...
    if (expediente) {
      ...
      this.configGlobal.getActive().pipe(
        ...
        switchMap(() => this.actoAdminService.getByNameAndTipoTramite('ILS_7_resolucion_concesion_sin_requerimiento', 'ILS'))
      ).subscribe({
        ...
        error: (err) => console.error('Error inicializando Res. Concesión Adhesión sin requerimiento', err)
      })
  ...
  getActoAdminDetail(): void {
    this.documentosGeneradosService.getDocumentosGenerados(this.actualID, this.actualNif, this.actualConvocatoria, 'doc_res_concesion_adhesion_sin_req_ils')
      .subscribe({
  ...
  sendActoAdminToSign(): void {
    ...
    const filename = `${this.actualIdExp}_${this.actualConvocatoria}_res_concesion_adhesion_sin_req_ils.pdf`;
  ...
  generateActoAdmin(): void {
    ...
    const docFieldToUpdate: string = "res_concesion_adhesion_sin_req_ils";
    ...
    rawTexto = rawTexto.replace(/%NOMBREPRESIDENTIDI%/g, this.nomPresidenteIdi);
    ...
    doc.setFont('helvetica', 'normal');
    doc.text(doc.splitTextToSize(jsonObject.resolucion_1, maxTextWidth), marginLeft + 5, 160);
    doc.text(doc.splitTextToSize(jsonObject.resolucion_2, maxTextWidth), marginLeft + 5, 172);
    doc.text(doc.splitTextToSize(jsonObject.resolucion_3, maxTextWidth), marginLeft + 5, 204);
    ...
    this.actoAdminService.sendPDFToBackEnd(formData).subscribe({
      next: (response) => {
        ...
        this.documentosGeneradosService.deleteByIdSolNifConvoTipoDoc(this.actualID, this.actualNif, String(this.actualConvocatoria), 'doc_res_concesion_adhesion_sin_req_ils')
          .subscribe({
            next: () => {
