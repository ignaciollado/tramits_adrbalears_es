...
export class ResolDesestimientoNoEnmendarIlsService {
  ...
  init(expediente: any, form: any) {
    ...
    if (expediente) {
      ...
      this.configGlobal.getActive().pipe(
        ...
        switchMap(() => this.actoAdminService.getByNameAndTipoTramite('ILS_2_resolucion_desestimiento_no_enmendar', 'ILS'))
      ).subscribe({
  ...
  getActoAdminDetail(): void {
    this.documentosGeneradosService.getDocumentosGenerados(this.actualID, this.actualNif, this.actualConvocatoria, 'doc_res_desestimiento_por_no_enmendar_ils')
      .subscribe({
  ...
  sendActoAdminToSign(): void {
    ...
    const filename = `${this.actualIdExp}_${this.actualConvocatoria}_res_desestimiento_por_no_enmendar_ils.pdf`;
  ...
  generateActoAdmin(): void {
    ...
    const docFieldToUpdate: string = "res_desestimiento_por_no_enmendar_ils";
    ...
    const formattedFecha_req_notif = formatDate(this.form.get('fecha_requerimiento_notif')?.value, 'dd/MM/yyyy', 'es-ES');
    ...
    rawTexto = rawTexto.replace(/%FECHANOTIFICACION%/g, formattedFecha_req_notif);
    ...
    doc.setFontSize(8);
    doc.text("Document: resolució desistiment per no esmena", marginLeft + 110, 45);
    ...
    doc.setFont('helvetica', 'normal');
    doc.text(doc.splitTextToSize(jsonObject.antecedentes_1_2_3_4, maxTextWidth), marginLeft + 5, 118);

    doc.setFont('helvetica', 'bold');
    doc.text(doc.splitTextToSize(jsonObject.resolucion, maxTextWidth), marginLeft, 190);
    doc.setFont('helvetica', 'normal');
    doc.text(doc.splitTextToSize(jsonObject.resolucion_1_2_3, maxTextWidth), marginLeft + 5, 198);
    ...
    doc.setFontSize(10);
    doc.setFont('helvetica', 'bold');
    doc.text(jsonObject.recursos, marginLeft, 60);
    doc.setFont('helvetica', 'normal');
    doc.text(doc.splitTextToSize(jsonObject.recursos_1, maxTextWidth), marginLeft, 68);
    doc.text(doc.splitTextToSize(jsonObject.recursos_2, maxTextWidth), marginLeft, 96);
    ...
    this.actoAdminService.sendPDFToBackEnd(formData).subscribe({
      next: (response) => {
        ...
        this.documentosGeneradosService.deleteByIdSolNifConvoTipoDoc(this.actualID, this.actualNif, this.actualConvocatoria, 'doc_res_desestimiento_por_no_enmendar_ils')
          .subscribe({
            next: () => {
