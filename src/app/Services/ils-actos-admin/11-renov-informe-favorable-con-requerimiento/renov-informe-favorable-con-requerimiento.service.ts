...
export class RenovInformeFavorableConRequerimientoIlsService {
  ...
  init(expediente: any, form: any) {
    ...
    if (expediente) {
      ...
      this.configGlobal.getActive().pipe(
        ...
        switchMap(() => this.actoAdminService.getByNameAndTipoTramite('ILS_11_renovacion_informe_favorable_con_requerimiento', 'ILS'))
      ).subscribe({
        ...
        error: (err) => console.error('Error inicializando Renov. Informe Favorable con requerimiento', err)
      })
  ...
  getActoAdminDetail(): void {
    this.documentosGeneradosService.getDocumentosGenerados(this.actualID, this.actualNif, this.actualConvocatoria, 'doc_renovacion_informe_favorable_con_req_ils')
      .subscribe({
  ...
  sendActoAdminToSign(): void {
    ...
    const filename = `${this.actualIdExp}_${this.actualConvocatoria}_renovacion_informe_favorable_con_req_ils.pdf`;
  ...
  generateActoAdmin(): void {
    ...
    const docFieldToUpdate: string = "renovacion_informe_favorable_con_req_ils";
    ...
    doc.setFontSize(8);
    doc.text("Document: renovació informe favorable", marginLeft + 110, 45);
    ...
    doc.text(doc.splitTextToSize(jsonObject.hechos_tit, maxTextWidth), marginLeft, 92);
    doc.setFont('helvetica', 'normal');
    doc.text(doc.splitTextToSize(jsonObject.hechos_1_2_3_4, maxTextWidth), marginLeft + 5, 100);

    doc.setFont('helvetica', 'bold');
    doc.text(doc.splitTextToSize(jsonObject.conclusion_tit, maxTextWidth), marginLeft, 164);
    doc.setFont('helvetica', 'normal')
    ...
    this.actoAdminService.sendPDFToBackEnd(formData).subscribe({
      next: (response) => {
        ...
        this.documentosGeneradosService.deleteByIdSolNifConvoTipoDoc(this.actualID, this.actualNif, String(this.actualConvocatoria), 'doc_renovacion_informe_favorable_con_req_ils')
          .subscribe({
            next: () => {
