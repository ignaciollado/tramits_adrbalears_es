...
export class RenovInformeFavorableIlsService {
  ...
  init(expediente: any, form: any) {
    ...
    if (expediente) {
      ...
      this.configGlobal.getActive().pipe(
        ...
        switchMap(() => this.actoAdminService.getByNameAndTipoTramite('ILS_10_renovacion_informe_favorable', 'ILS'))
      ).subscribe({
  ...
  getActoAdminDetail(): void {
    this.documentosGeneradosService.getDocumentosGenerados(this.actualID, this.actualNif, this.actualConvocatoria, 'doc_renovacion_informe_favorable_ils')
      .subscribe({
  ...
  sendActoAdminToSign(): void {
    ...
    const filename = `${this.actualIdExp}_${this.actualConvocatoria}_renovacion_informe_favorable_ils.pdf`;
  ...
  generateActoAdmin(): void {
    ...
    const docFieldToUpdate: string = "renovacion_informe_favorable_ils";
    ...
    const formattedFecha_REC_justificacion_renov = formatDate(this.form.get('fecha_REC_justificacion_renov')?.value, 'dd/MM/yyyy HH:mm:ss', 'es-ES');
    ...
    doc.setFont('helvetica', 'normal');
    doc.text(doc.splitTextToSize(jsonObject.hechos_1_2_3, maxTextWidth), marginLeft + 5, 100);

    doc.setFont('helvetica', 'bold');
    doc.text(doc.splitTextToSize(jsonObject.conclusion_tit, maxTextWidth), marginLeft, 154);
    doc.setFont('helvetica', 'normal');
    doc.text(doc.splitTextToSize(jsonObject.conclusion_txt, maxTextWidth), marginLeft, 162);
