...
export class RequerimientoIlsService {
  ...
  init(expediente: any, form: any) {
    ...
    if (expediente) {
      ...
      this.configGlobal.getActive().pipe(
        ...
        switchMap(() => this.actoAdminService.getByNameAndTipoTramite('ILS_1_requerimiento', 'ILS'))
      ).subscribe({
            ...
            this.expedienteService.updateFieldExpediente(this.actualID, 'fecha_requerimiento', this.commonService.convertUnixToHumanDate(resp.endDate, true))
              .subscribe({
                next: () => {
                  ...
                  this.expedienteService.updateFieldExpediente(this.actualID, 'situacion', 'reqFirmado').subscribe({
                    next: () => {
  ...
  sendActoAdminToSign(): void {
    ...
    const filename = `${this.actualIdExp}_${this.actualConvocatoria}_requeriment_ils.pdf`;
  ...
  saveReasonRequest(formRequerimiento: any): void {
    ...
    if (formRequerimiento.valid) {
      this.motivoRequerimiento = motivo;
      ...
      this.documentosGeneradoService.deleteByIdSolNifConvoTipoDoc(this.actualID, this.actualNif, this.actualConvocatoria, 'doc_requeriment_ils').subscribe({
  ...
  generateActoAdmin(): void {
    const docFieldToUpdate: string = 'requeriment_ils';
    ...
    doc.text(doc.splitTextToSize(`    • ${this.motivoRequerimiento?.trim() || this.form.get('motivoRequerimiento')?.value}`, maxTextWidth), marginLeft, 125);
