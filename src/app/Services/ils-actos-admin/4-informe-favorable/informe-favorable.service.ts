...
export class InformeFavorableIlsService {
  ...
  init(expediente: any, form: any) {
    ...
    if (expediente) {
      ...
      this.configGlobal.getActive().pipe(
        ...
        switchMap(() => this.actoAdminService.getByNameAndTipoTramite('ILS_4_informe_favorable_sin_requerimiento', 'ILS'))
      ).subscribe({
        ...
        error: (err) => console.error('Error inicializando Informe Favorable', err)
      })
  ...
  getActoAdminDetail(): void {
    this.documentosGeneradosService.getDocumentosGenerados(this.actualID, this.actualNif, this.actualConvocatoria, 'doc_informe_favorable_sin_requerimiento_ils')
      .subscribe({
  ...
  sendActoAdminToSign(): void {
    ...
    const filename = `${this.actualIdExp}_${this.actualConvocatoria}_informe_favorable_sin_requerimiento_ils.pdf`;
  ...
  generateActoAdmin(): void {
    ...
    const docFieldToUpdate: string = "informe_favorable_sin_requerimiento_ils";
    ...
    doc.setFontSize(8);
    doc.text("Document: informe favorable", marginLeft + 110, 45);
    ...
    doc.setFont('helvetica', 'bold')
    doc.text(jsonObject.conclusion_tit, marginLeft, 140);
    doc.setFont('helvetica', 'normal');
    doc.text(doc.splitTextToSize(jsonObject.conclusion_txt, maxTextWidth), marginLeft, 148);
    ...
    this.actoAdminService.sendPDFToBackEnd(formData).subscribe({
      next: (response) => {
        ...
        this.documentosGeneradosService.deleteByIdSolNifConvoTipoDoc(this.actualID, this.actualNif, String(this.actualConvocatoria), 'doc_informe_favorable_sin_requerimiento_ils')
          .subscribe({
            next: () => {
