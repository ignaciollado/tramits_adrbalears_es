...
export class ResolConcesionAdhesionConRequerimientoIlsService {
  ...
  init(expediente: any, form: any) {
    ...
    if (expediente) {
      ...
      this.configGlobal.getActive().pipe(
        ...
        switchMap(() => this.actoAdminService.getByNameAndTipoTramite('ILS_8_resolucion_concesion_con_requerimiento', 'ILS'))
      ).subscribe({
        ...
        error: (err) => console.error('Error inicializando Res. Concesión Adhesión con requerimiento', err)
      })
  ...
  getActoAdminDetail(): void {
    this.documentosGeneradosService.getDocumentosGenerados(this.actualID, this.actualNif, this.actualConvocatoria, 'doc_res_concesion_adhesion_con_req_ils')
      .subscribe({
  ...
  sendActoAdminToSign(): void {
    ...
    const filename = `${this.actualIdExp}_${this.actualConvocatoria}_res_concesion_adhesion_con_req_ils.pdf`;
  ...
  generateActoAdmin(): void {
    ...
    const docFieldToUpdate: string = "res_concesion_adhesion_con_req_ils";
    ...
    const formattedFecha_maxima_enmienda = formatDate(this.form.get('fecha_maxima_enmienda')?.value, 'dd/MM/yyyy', 'es-ES');
    const formattedFecha_infor_fav = formatDate(this.form.get('fecha_infor_fav')?.value, 'dd/MM/yyyy', 'es-ES');
    const formattedFecha_adhesion_ils = formatDate(this.form.get('fecha_adhesion_ils')?.value, 'dd/MM/yyyy', 'es-ES');
    const formattedFecha_seguimiento_adhesion_ils = formatDate(this.form.get('fecha_seguimiento_adhesion_ils')?.value, 'dd/MM/yyyy', 'es-ES');
    ...
    rawTexto = rawTexto.replace(/%FECHANOTIFIC%/g, formattedFecha_requerimiento_notif);
    rawTexto = rawTexto.replace(/%FECHAENMIENDA%/g, formattedFecha_maxima_enmienda);
    rawTexto = rawTexto.replace(/%FECHAINFORMEFAV%/g, formattedFecha_infor_fav);
    rawTexto = rawTexto.replace(/%FECHADESDECONCESION%/g, formattedFecha_adhesion_ils);
    rawTexto = rawTexto.replace(/%FECHAHASTACONCESION%/g, formattedFecha_seguimiento_adhesion_ils);
    ...
    doc.setFontSize(8);
    doc.text("Document: resolució concessió adhesió", marginLeft + 110, 45);
    ...
    doc.setFont('helvetica', 'normal');
    doc.text(doc.splitTextToSize(jsonObject.antecedentes_1_2_3_4_5, maxTextWidth), marginLeft + 5, 100);

    doc.setFont('helvetica', 'bold');
    doc.text(doc.splitTextToSize(jsonObject.resolucion, maxTextWidth), marginLeft, 172);
    doc.setFont('helvetica', 'normal');
    doc.text(doc.splitTextToSize(jsonObject.resolucion_1, maxTextWidth), marginLeft + 5, 180);
    doc.text(doc.splitTextToSize(jsonObject.resolucion_2, maxTextWidth), marginLeft + 5, 192);
    doc.text(doc.splitTextToSize(jsonObject.resolucion_3, maxTextWidth), marginLeft + 5, 224);
    ...
    doc.setFontSize(10);
    doc.setFont('helvetica', 'bold');
    doc.text(doc.splitTextToSize(jsonObject.inter_recursos, maxTextWidth), marginLeft, 60);
    doc.setFont('helvetica', 'normal');
    doc.text(doc.splitTextToSize(jsonObject.inter_recursos_texto1, maxTextWidth), marginLeft, 68);
    doc.text(doc.splitTextToSize(jsonObject.inter_recursos_texto2, maxTextWidth), marginLeft, 96);
    ...
    this.actoAdminService.sendPDFToBackEnd(formData).subscribe({
      next: (response) => {
        ...
        this.documentosGeneradosService.deleteByIdSolNifConvoTipoDoc(this.actualID, this.actualNif, String(this.actualConvocatoria), 'doc_res_concesion_adhesion_con_req_ils')
          .subscribe({
            next: () => {
  ...
  private tieneTodosLosCamposRequeridos(): void {
    ...
    const fecha_infor_fav = this.form.get('fecha_infor_fav')?.value;
    const fecha_adhesion_ils = this.form.get('fecha_adhesion_ils')?.value;
    const fecha_seguimiento_adhesion_ils = this.form.get('fecha_seguimiento_adhesion_ils')?.value;
    ...
    if (!fecha_infor_fav?.trim() || fecha_infor_fav?.trim() === "0000-00-00") {
      this.camposVacios.push('FORM.FAVORABLE-DATE')
    }

    if (!fecha_adhesion_ils?.trim() || fecha_adhesion_ils?.trim() === "0000-00-00") {
      this.camposVacios.push('FORM.ADHESION-DATE')
    }

    if (!fecha_seguimiento_adhesion_ils?.trim() || fecha_seguimiento_adhesion_ils?.trim() === "0000-00-00") {
      this.camposVacios.push('FORM.TRACKING-DATE')
    }

    if (!fecha_maxima_enmienda?.trim() || fecha_maxima_enmienda?.trim() === "0000-00-00") {
      this.camposVacios.push('FORM.FECHA_MAXIMA_ENMIENDA')
    }
